...
RandomizedSet.prototype.getNextRandom = function(orderOfMagnitude) {
  const nextRand = this.random(orderOfMagnitude);
  while (this.insert(nextRand) === false) nextRand = this.random(orderOfMagnitude);
  return nextRand;
};


RandomizedSet.prototype.random = function(digits) {
  let orderOfMagnitude = Math.pow(10, digits);
  return Math.floor(Math.random() * orderOfMagnitude);
}
